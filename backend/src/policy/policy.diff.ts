...
export function diffPacks(from: PolicyPack, to: PolicyPack): PolicyChange[] {
  ...
  const flag = (
    ...
    out.push({ path, label, from: a ? "on" : "off", to: b ? "on" : "off", direction: b ? onIs : inverse });
  };
  ...
  flag("rules.sentiment", "Negative-sentiment halt", from.rules.sentiment, to.rules.sentiment, "tighter");
  flag("rules.deadline", "Deadline expiry", from.rules.deadline, to.rules.deadline, "tighter");
  flag("rules.attempt_cap", "Max-attempts exhaustion", from.rules.attempt_cap, to.rules.attempt_cap, "tighter");

  num("sentimentThreshold", "Sentiment halt threshold", from.sentimentThreshold, to.sentimentThreshold, ratio, "looser");

  num("escalation.discountCapPercent", "Discount a human may approve", from.escalation.discountCapPercent, to.escalation.discountCapPercent, percent, "looser");
  ...
  num("escalation.confidenceFloor", "Confidence floor", from.escalation.confidenceFloor, to.escalation.confidenceFloor, ratio, "tighter");
  ...
  num("mandate.spacingDays", "Days between re-presentations", from.mandate.spacingDays, to.mandate.spacingDays, days, "tighter");
  ...
  for (const channel of POLICY_CHANNELS) {
    flag(`channels.${channel}`, `${CHANNEL_LABELS[channel]} channel`, from.channels[channel], to.channels[channel], "looser");
  }
